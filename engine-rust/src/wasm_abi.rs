...
use crate::{
    run_json, run_public_aircraft_reference_json, run_sixdof_verification_json, EngineError,
    MAX_INPUT_BYTES,
};
...
/// Execute the isolated versioned six-DOF numerical verification kernel.
#[allow(unsafe_code)]
#[no_mangle]
pub extern "C" fn vector_sixdof_verification_run_json() -> u32 {
    let result = INPUT.with(|input| {
        let input = input.borrow();
        std::str::from_utf8(&input)
            .map_err(|_| EngineError::InvalidUtf8)
