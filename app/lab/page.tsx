...
function LabWorkbench({
  ...
  const draftAdmissionTracker = useMemo(() => new ScenarioDraftAdmissionTracker(), []);
  useEffect(() => {
    ...
    fetch("/api/catalog")
      ...
      .then(async (data: unknown) => {
        ...
        if (active) {
          ...
          const admittedScenario = { ...template.package.scenario };
          scenarioRef.current = admittedScenario;
          draftAdmissionTracker.invalidate();
          setScenario(admittedScenario);
          setDraftRevision(0);
  ...
  }, [draftAdmissionTracker, initialDefinition.id, initialDefinition.version]);
  ...
  >((action) => {
    setScenario((current) => {
      ...
      const synchronized = next.airMission !== current.airMission
        ? next
        : synchronizeScenarioAirMission(next, CURRENT_COMPILED_MODEL_PACK);
      scenarioRef.current = synchronized;
      draftAdmissionTracker.invalidate();
      return synchronized;
    });
  ...
  const run = useCallback(async () => {
    ...
    const submittedScenario = structuredClone(scenario);
    let admissionStarted = false;
    try {
      setSaveError(null);
      setRunProgress(0);
      setRuntimeState("initialization");
      const admission = await draftAdmissionTracker.begin(
        submittedScenario,
      ...
      if (error instanceof ScenarioDraftAdmissionError) {
        ...
        setVectorRecord(null);
        setWorkspace("configure");
        setSaveError("Configuration changed while admission or simulation was in flight. The stale result was discarded; run again.");
        return;
      }
      ...
      if (admissionStarted) draftAdmissionTracker.invalidate();
    }
    setTime(0);
    setPlaying(true);
    setWorkspace("run");
  ...
  return (
    <main className="lab-shell">
      ...
      {saveError && workspace !== "results" && (
        <p className="save-error workbench-error" role="alert">{saveError}</p>
      )}
