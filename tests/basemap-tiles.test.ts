...
test("equivalent tile tuples use a single cache identity regardless of parameter order", () => {
  const fields = ["mode=minimal", "z=3", "x=2", "y=1"];
  ...
  assert.deepEqual(identities, new Set(["vector-basemap-tile.v1/minimal/3/2/1"]));
});
...
test("unknown, duplicate, encoded, empty, leading-zero, and out-of-range input rejects before cache or upstream", async () => {
  for (const query of [
    ...
    await assert.rejects(
      serveBasemapTile(request(query), fixture.values),
...
test("concurrent identical cache misses use one bounded upstream request", async () => {
  ...
  const responses = await Promise.all(Array.from({ length: 128 }, () => serveBasemapTile(request("mode=tactical&z=3&x=2&y=1"), fixture.values)));
...
test("cache expiry uses the declared clock and a cache schema key", async () => {
  const fixture = dependencies();
  await serveBasemapTile(request("mode=standard&z=3&x=2&y=1"), fixture.values);
  await serveBasemapTile(request("y=1&mode=standard&x=2&z=3"), fixture.values);
  assert.equal(fixture.upstreamCalls, 1);
...
test("non-images, oversized content, failed upstreams, and timeouts do not enter cache", async () => {
  ...
  for (const result of cases) {
    ...
    await assert.rejects(
      serveBasemapTile(request("mode=minimal&z=3&x=2&y=1"), {
        cache,
        now: () => 1_000,
        fetch: async () => {
...
test("cache adapters preserve the same response contract in Node and Worker-shaped runtimes", async () => {
  for (const cache of [new FakeCache(), new FakeCache()]) {
    ...
    const response = await serveBasemapTile(request("mode=minimal&z=3&x=2&y=1"), fixture.values);
    assert.equal(response.headers.get("x-vector-cache-schema"), "vector-basemap-tile.v1");
...
test("a failing cache adapter does not bypass canonical bounds or block a valid bounded upstream response", async () => {
  ...
  const response = await serveBasemapTile(request("mode=minimal&z=3&x=2&y=1"), values);
  assert.equal(response.status, 200);
  assert.equal(upstreamCalls, 1);
  await assert.rejects(serveBasemapTile(request("mode=minimal&z=3&x=2&y=1&extra=1"), values));
  assert.equal(upstreamCalls, 1);
});
