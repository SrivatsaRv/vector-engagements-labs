...
It verifies exact sizes and hashes, both NASA metadata rights/export facts, FAA MD5, PDF page counts, all declared render hashes, the rejected alternate, manifest/source/render digests, and all 21 available binary-isolation fingerprints. `npm run policy:generic-mission-policy-source:verify` is the source-byte-independent CI contract/tamper/isolation gate. Across every production and runtime-fixture root it rejects whole-file SHA-256 matches plus SHA-256-confirmed rolling fingerprints of raw and contiguous-base64 copies of all 24 governed PDFs, metadata responses, page renders, and contact sheets. It cannot claim exact-source reproduction; the completion evidence must record the explicit external gate separately.

The committed production-isolation record is a fail-closed `POLICY_TEMPLATE`, never reusable PASS evidence. Each invocation emits a separate `PASS` report containing the exact candidate and runtime Git heads, tracked-change state, the content digest of every attested verifier/policy/input, and the content digest of the complete scanned production tree. The report itself is not committed, avoiding a self-referential commit field while making the tested candidate and byte state explicit in local and hosted logs.
