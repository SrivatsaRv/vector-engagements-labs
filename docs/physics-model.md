...
kernel with a strict TypeScript/Rust-WASM contract. Its Rust implementation is
compiled into a separate verification-only artifact that the production engine,
backend, and Worker cannot load. It proves rigid-body equation, quaternion,
conservation, convergence, rejection, and parity foundations only; it is not
wired to the production scenario engine and does not upgrade aircraft fidelity.
See [`sixdof-numerical-foundation.md`](sixdof-numerical-foundation.md).
...
applicability. All reference-use, redistribution, export, and human visual QA
states remain pending, so the source cannot be compiled or executed.
