...
On Apple M5 / macOS arm64 / Node v24.3.0, three isolated final 20-batch runs (15 cases, 12,145 frames, approximately 11.0 MB JSON per batch) observed TypeScript p95 23.220–25.989 ms and Node-hosted Rust-WASM p95 158.133–177.073 ms. A ten-run pre-change calibration on the same named host observed TypeScript p95 20.629–25.676 ms and Rust-WASM p95 157.117–160.720 ms; the post-policy ten-run recheck observed 21.199–25.996 ms and 159.438–164.396 ms respectively. The closed `APPLE_M5_NODE24` regression profile therefore retains 30 and 200 ms. Its bound identity is exact: Node runtime, Darwin platform, arm64 architecture, `Apple M5` CPU, 10 logical cores, 17,179,869,184 memory bytes, `githubActions=false`, and null runner OS, runner architecture, image OS, and image version must all be present and unchanged. Two prior TypeScript p95 results of 35.614 and 119.161 ms remain excluded as contaminated because concurrent clean-clone Vitest workers occupied 70–90% CPU across eight processes.

Hosted Stage 2B now selects the separate closed `GITHUB_HOSTED_UBUNTU24_X64_NODE22` profile explicitly. The unchanged exact-`d9023793…` diagnostic campaign on run `32727846075`, Ubuntu 24.04 image `20260816.277.1`, Linux x64 and Node v22.18.0 observed TypeScript p95 46.208, 52.163, 45.081, and 34.857 ms in the original job `97433307768` and sequential diagnostic jobs `97435257856`, `97436000926`, and `97436705551`. These failed reruns are calibration evidence, never release evidence. The hosted TypeScript ceiling is 65 ms, 24.6 percent above the largest same-profile calibration sample.

Exact candidate `23ee24fea6d44f67cbbc7081a3bfadd0d854097a` then supplied two complete sequential hosted Rust-WASM distributions. Superseded run `32751410825`, job `97509066963`, measured TypeScript samples from 30.246 to 51.357 ms with p95 47.492 ms and Rust-WASM samples from 288.741 to 305.220 ms with p95 303.618 ms. Authoritative run `32751532797`, job `97509842527`, measured TypeScript samples from 29.370 to 48.767 ms with p95 43.440 ms and Rust-WASM samples from 289.709 to 303.386 ms with p95 300.864 ms. Both used the identical bound profile identity and the identical observed context: Ubuntu 24.04.4, Node v22.18.0, Linux x64, GitHub Actions runner OS `Linux`, runner architecture `X64`, `ImageOS=ubuntu24`, AMD EPYC 9V74, 4 logical cores, 16,766,414,848 memory bytes, and `ImageVersion=20260816.277.1`. The tightly overlapping Rust-WASM results establish a hosted-only 380 ms ceiling, 25.16 percent above the largest same-profile p95. Apple M5 remains 30/200 ms and hosted TypeScript remains 65 ms. This is calibration-only Node-hosted evaluator evidence: it is not a browser Worker, product capacity, entity capacity, or cross-hardware performance claim.

The v3 benchmark measures both backends before evaluation and emits `boundProfileIdentity` separately from `observedContext`, followed by all 20 sorted samples, p50/p95/p99/maximum, RSS growth, frames, and bytes before an aggregate nonzero failure. The Apple profile binds every recorded host field and has an empty observation object. The hosted profile binds only Node runtime, platform, architecture, GitHub Actions state, runner OS/architecture, and `ImageOS`; CPU model, logical-core count, memory bytes, and `ImageVersion` remain labeled observations and cannot authorize that profile. A missing hosted observation is reported as `null` and does not change profile selection. Profile selection never auto-detects a more permissive ceiling; missing, unknown, or bound-identity-mismatched profiles reject, and the removed `VECTOR_MAX_GENERIC_AAM_*_P95_MS` variables cannot override either closed policy. The benchmark makes no Worker, UI, 100-entity, or product-capacity claim. A regression-first canonical-text digest implementation produced a 1,366.461 ms TypeScript p95 and was rejected; the admitted fixed-order binary64 output encoding preserves all governed fields.
