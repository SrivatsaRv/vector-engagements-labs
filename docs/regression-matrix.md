...
| Generic AAM verification | NASA TM-109057 byte/corpus/workload tamper rejection; exact input/run contracts; independent LOS, closest-approach, mass, lag, boundary and convergence oracles; forward/reversed 15-case actual-WASM full-frame parity; no production imports | None: `ENGINE_VERIFICATION_ONLY`; no Worker, UI, event, VSR, named weapon, or Situation Log consumer |
...
| Map authoring | Map/scalar state equivalence, heading/speed compilation, route retention, route-origin integrity, fly-by radius TS/Rust/VSR parity and trajectory contrast, zero-length-leg and preset-boundary rejection; invalid intermediate text is preserved and blocks Run | Two draggable starts, numeric WGS84/MSL start and waypoint editor with visible `vector.route-plan.v1` capture constraint, affiliation-scoped base-origin pickers, selectable installations, scoped waypoint action, six custom navigation controls, persisted basemap switch, resize recovery, real-Worker browser run, no browser error overlay |
| Symbols | 7 entity kinds × 3 affiliations × 5 lifecycle states; deterministic projected-label collision/selection policy | Map and 3D use the same symbol contract; keyboard marker selection discloses a full label without changing playback |
...
- `tests/vector-record.test.mjs` proves direct `events.jsonl` persistence,
  ordering and causal corruption rejection, unknown payload kind/version
  rejection, unsupported envelope rejection, and explicit read-only
  unavailability for legacy v1 events.
