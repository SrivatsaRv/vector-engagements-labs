...
check, native tests, rustdoc, the full reference verifier, and its explicitly
profiled bounded performance check. Stage 2C audits the crate's independent lockfile. The
...
The shared canonical-builder regression uses a non-root host identity, requires
an EXIT ownership-restoration trap confined to `/target`, and proves temporary
target removal after both successful and failed container execution.
