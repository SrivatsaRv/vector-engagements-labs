...
export function LandingMiniSim() {
  ...
  const definition = SCENARIO_LIBRARY.find((item) => item.domain === domain) ?? SCENARIO_LIBRARY[0];
  ...
  return <section className="landing-sim" aria-label="Playable engagement model preview">
    <header className="landing-sim-header">
      <div><span className="live-dot"/>Example · Live model</div>
      <strong>{definition.title}</strong>
      <time>{time.toFixed(1).padStart(4, "0")} s</time>
    </header>
    <nav className="landing-sim-domains" aria-label="Engagement domain">
      {DOMAINS.map((item) => <button key={item} className={domain === item ? "active" : ""} onClick={() => { setDomain(item); setTime(0); setPlaying(true); }}><strong>{item}</strong><span>{DOMAIN_DETAILS[item].label}</span></button>)}
    </nav>
    ...
    <footer className="landing-sim-footer">
      <button aria-label={playing ? "Pause preview" : "Play preview"} onClick={() => setPlaying((value) => !value)}>{playing ? <Pause size={14}/> : <Play size={14}/>}</button>
      <button aria-label="Restart preview" onClick={() => setTime(0)}><RotateCcw size={14}/></button>
      <input aria-label="Preview timeline" type="range" min={0} max={result.timeOfFlight || 1} step={0.1} value={time} onChange={(event) => { setPlaying(false); setTime(Number(event.target.value)); }}/>
      <span>{frame.phase}</span>
      <Link href={`/lab?scenario=${definition.id}`}>Open full scenario</Link>
    </footer>
  </section>;
}
