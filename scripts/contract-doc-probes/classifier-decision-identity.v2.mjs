...
function materialize(root, commit, directory) {
  ...
  const parserPath = join(directory, "scripts", "lib", "git-name-status.mjs");
  ...
  const parserBytes = execFileSync("git", ["show", `${commit}:scripts/lib/git-name-status.mjs`], {
    cwd: root,
    maxBuffer: 8 * 1024 * 1024,
  });
  const moduleSourceSha256 = sha256(Buffer.concat([classifierBytes, Buffer.from([0]), parserBytes]));
  writeFileSync(classifierPath, classifierBytes);
  writeFileSync(parserPath, parserBytes);
  const realClassifierPath = realpathSync(classifierPath);
  const realParserPath = realpathSync(parserPath);
  invariant(
    sha256(Buffer.concat([readFileSync(realClassifierPath), Buffer.from([0]), readFileSync(realParserPath)])) === moduleSourceSha256,
    "Materialized classifier or name-status parser differs from its Git blob before execution.",
  );
  return { classifierPath: realClassifierPath, parserPath: realParserPath, moduleSourceSha256 };
}
...
function matrix(materialized, paths, revision, observerPath) {
  const { classifierPath, parserPath, moduleSourceSha256 } = materialized;
  ...
  invariant(
    sha256(Buffer.concat([readFileSync(classifierPath), Buffer.from([0]), readFileSync(parserPath)])) === moduleSourceSha256,
    "Classifier module or name-status parser changed during execution.",
  );
  return {
    moduleSourceSha256,
...
async function main() {
  ...
  invariant(probeId === "DELIVERY_CLASSIFIER_DECISION_IDENTITY_V2", "Classifier probe identity mismatch.");
